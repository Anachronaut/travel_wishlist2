...
def place_list(request):
    ...
    in the form. If new place is valid, it will save a new place to
    the db, and redirect to this same page.

    This creates a GET request to the same route.

    If not a POST route, or new place is not valid, displays a list
    of places and a form to add a new place.
    """

    if request.method == 'POST':
        form = NewPlaceForm(request.POST)
        place = form.save(commit=False)
        place.user = request.user #associate place with current user
        if form.is_valid():
            place.save()
